...
class MotorController:
    def __init__(
        self, port: str, slave_id: int = 1, baudrate: int = 115200, timeout: float = 1.0
    ) -> None:
...
if __name__ == "__main__":
    ...
    PORT = "/dev/ttyUSB0"  # "COM3" for windows
    SLAVE_ID = 1
    ...
    try:
        ...
        for i in range(5):
            ...
            # 阶段1: 运动到位置 9000 (闭合?)
            sdk.temp_move(
                position_mm=9000, speed_pct=50, force_pct=25, accel=60, decel=60, trigger=True
            )
            time.sleep(3)  # 等待运动完成

            # 阶段2: 运动到位置 0 (张开?)
            sdk.temp_move(
                position_mm=0, speed_pct=50, force_pct=25, accel=60, decel=60, trigger=True
            )
            time.sleep(3)  # 等待运动完成
